...
# ⑥ 查看用户的记忆统计（操作日志、历史会话数等）
print(session.stats())   # {"total_memories": 3, "sessions": 1}
```
...
| `turn` | `int` | 本轮轮次号 |
| `flush_stats` | `dict \| None` | 仅当 `flush_after_turn=True` 时有值，格式同 `flush()` 返回值 |
| `messages` | `list` | 本轮发送给 LLM 的完整 messages 列表，可用于调试 |
...
> 💡 使用 `MemoryChatSession` 时，可选择等价劋捷方式：`session.stats()`，无需单独传入 `user_id`。
...
config.MEMORY_TOKEN_BUDGET = 2000        # 覆盖默认值 1200（适合长上下文模型）
